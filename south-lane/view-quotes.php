...
if (isset($_GET['contact_number'])) {
    $urlToHit=$_SERVER['SERVER_NAME'].'/south-lane/api/quotes/read.php?contact_number='.urlencode($_GET['contact_number']);
    if (isset($_GET['pending']) && $_GET['pending']=="true") {
        $urlToHit.="&pending=true";
    }
}else{
    $urlToHit=$_SERVER['SERVER_NAME'].'/south-lane/api/quotes/read.php';
    if (isset($_GET['pending']) && $_GET['pending']=="true") {
        $urlToHit.="?pending=true";
    }
}
...
<script>
    function editReceiptDetails(obj,tdClass){
        ...
        var buttonHtml='<p style="color:green;" onclick="savequotesDetails(this,`'+tdClass+'`)" class="edit-paragraph">Save <i class="fa fa-copy"></i></p>';
        $(currentTr).find('td.'+tdClass).html(saveHtml);
        $(currentTr).find('td.'+tdClass+'_button').html(buttonHtml);
    }

    function savequotesDetails(obj,tdClass){
        var currentTr=$(obj).closest("tr");
        var id =$(currentTr).find('td.quote_id_unique').text();
        var currentTr=$(obj).closest("tr");
        ...
        var settings = {
            "url": "./api/quotes/update.php?id="+id,
            "method": "POST",
            "timeout": 0,
            "headers": {
        ...
        $.ajax(settings).done(function (response) {
            ...
            Swal.fire(
                'Good job',
                'quotess Details updated successfully',
                'success'
            )
            ...
            <div class="row">
                <div class="col">
                    <div class="section_title_container text-center">
                        <h2 class="section_title">View Quote Requests</h2>
                    </div>
                </div>
            </div>
        </div>
        <div class="container" style="margin-top:50px; overflow-x:scroll;">
            ...
            <tbody>
                ...
                <tr class="template">
                    ...
                    <td class="quote_id_unique"><?=$value->quote_id_unique?></td>
